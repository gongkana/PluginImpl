...
public class SMS4 {
    ...
    public static byte[] encodeSMS4(byte[] plaintext, byte[] key) throws DataException {
    	if (null == key || key.length != 16){
    		...
    		throw new DataException(-103,"国密密钥不对");
    	}
 		int len = plaintext.length;

		byte[] bef = new byte[16];
		byte[] ret = new byte[(len + 15) / 16 * 16];

		int count = plaintext.length / 16;

		for (int i = 0; i < count; i++) {
			System.arraycopy(plaintext, i * 16, bef, 0, 16);
			byte[] dst = encode16(bef, key);

			System.arraycopy(dst, 0, ret, i * 16, 16);
		}

		if ((len % 16) != 0) {
			Arrays.fill(bef, (byte)0x00);
			System.arraycopy(plaintext, count * 16, bef, 0, len % 16);
			byte[] dst = encode16(bef, key);
			System.arraycopy(dst, 0, ret, count * 16, 16);
		}
		return ret;
    }
    ...
    public static byte[] decodeSMS4(byte[] ciphertext, byte[] key) throws DataException  {
		...
		int count = ciphertext.length / 16;

		for (int i = 0; i < count; i++) {
			System.arraycopy(ciphertext, i * 16, bef, 0, 16);
			byte[] dst = decode16(bef, key);
		...
		if ((len % 16) != 0) {
			Arrays.fill(bef, (byte)0x00);
			System.arraycopy(ciphertext, count * 16, bef, 0, len % 16);
			byte[] dst = decode16(bef, key);
			System.arraycopy(dst, 0, ret, count * 16, 16);
		}
