package com.nantian.sign.views;
...
import com.nantian.sign.utils.Bezier;
import com.nantian.sign.utils.ControlTimedPoints;
import com.nantian.sign.utils.SvgBuilder;
...
public class SignaturePad extends View {
    ...
    // Cache
    private ArrayList<TimedPoint> mPointsCache = new ArrayList<TimedPoint>();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mLastTouchX = eventX;
                mLastTouchY = eventY;
                TimedPoint  point = getNewPoint(eventX, eventY,event.getPressure(),event.getEventTime());
                addPoint(point);
                prePoint = point;
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                for (int i = 0;i<N;i++){
                    ...
                    if (Math.abs(x-prePoint.x)<1 || Math.abs(y-prePoint.y)<1){
                        prePoint = new TimedPoint(x, y,event.getHistoricalPressure(0),event.getHistoricalEventTime(0));
                    }else{
                        resetDirtyRect(x, y);
    ...
    private TimedPoint getNewPoint(float x, float y,float pre,long time) {
        ...
        return timedPoint.set(x, y,pre,time);
    }
    ...
    private void addPoint(TimedPoint newPoint) {
        ...
        if (pointsCount > 3) {
            ...
            */
            ...
            float min = mMaxWidth*endPoint.presure;
            min = min>2f?min:2f;
            float max = mMaxWidth*startPoint.presure;
            if (lastWidth != 0){
                max = max<min?lastWidth:max;
            }
            addBezier(curve, max, min);
            lastWidth = min;
           ...
           // mPoints.add(getNewPoint(firstPoint.x, firstPoint.y,firstPoint.presure,firstPoint.timestamp));
        }
    }
    ...
    private ControlTimedPoints calculateCurveControlPoints(TimedPoint s1, TimedPoint s2, TimedPoint s3) {
        ...
        float tx = s2.x - cmX;
        float ty = s2.y - cmY;
        return mControlTimedPointsCached.set(getNewPoint(m1X + tx, m1Y + ty,(s1.presure+s2.presure)/2.0f, System.currentTimeMillis()),
                getNewPoint(m2X + tx, m2Y + ty,(s3.presure+s2.presure)/2.0f, System.currentTimeMillis()));
    }
